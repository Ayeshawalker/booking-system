...
if ! curl --silent --fail "http://localhost:$PORT/" >/dev/null 2>&1; then
  ...
  nohup python3 -m http.server "$PORT" --bind 127.0.0.1 >/tmp/ayesha-booking-server.log 2>&1 &
  sleep 1
fi

if ! curl --silent --fail "http://localhost:$RECOVERY_PORT/" >/dev/null 2>&1; then
  cd "$PROJECT_DIR/recovery-redirect" || exit 1
  nohup python3 -m http.server "$RECOVERY_PORT" --bind 127.0.0.1 >/tmp/ayesha-recovery-server.log 2>&1 &
  sleep 1
fi
