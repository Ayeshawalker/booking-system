...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (instruction === "extract-abc") {
      const abcInstructions = `You assist a UK therapist by extracting a draft CBT/REBT ABC formulation from their own session note. Use only information explicitly supported by the note. Never diagnose, infer motives, invent details, add treatment recommendations or make autonomous clinical decisions. If a component is absent or ambiguous, write exactly "Not clearly stated in the note." Keep each component concise and clinically neutral. Return valid JSON only with this exact shape: {"activatingEvent":"...","beliefs":"...","consequences":"..."}. The output is a suggestion requiring mandatory clinician review.`;
