...
function confirmationMessage(booking:Booking,clientZone=timeZone) {
  ...
  const hasInPersonSession=items.some((item)=>item.format==="In person");
  const accessReminder="Access reminder for in-person sessions: When you arrive, please enter *8733* into the console at the metal gates. Once through the gates, go to the entrance of the Cherry Tree Centre and enter the room number into the console by the door. This will alert me so I can let you into the building.";
  const text=[greeting(booking),"",intro,"",...lines,hasInPersonSession?"":null,hasInPersonSession?accessReminder:null,"","If you need to get in touch or rearrange your appointment, simply reply to this email. If you do need to make a change, if possible please give me at least 48 hours’ notice to avoid a cancellation charge.","","I look forward to seeing you then.","","Warm wishes,","Ayesha"].filter((line)=>line!==null).join("\n");
  ...
  const accessHtml=hasInPersonSession?`<div style="margin:20px 0;padding:16px;border-radius:10px;background:#fff0d2"><strong style="color:#b71368">Access reminder for in-person sessions</strong><p style="margin:8px 0 0">When you arrive, please enter <strong>*8733*</strong> into the console at the metal gates. Once through the gates, go to the entrance of the Cherry Tree Centre and enter the room number into the console by the door. This will alert me so I can let you into the building.</p></div>`:"";
  const html=emailShell(intro,greeting(booking),`<p style="margin:0 0 14px">${escapeHtml(intro)}</p>${itemHtml}${accessHtml}<p style="margin:22px 0 16px">If you need to get in touch or rearrange your appointment, simply reply to this email. If you do need to make a change, if possible please give me at least 48 hours’ notice to avoid a cancellation charge.</p><p style="margin:0">I look forward to seeing you then.</p>`);
