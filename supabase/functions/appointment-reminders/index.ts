...
function confirmationMessage(booking:Booking) {
  ...
  const lines=items.map((item)=>`• ${displayDate(item.date)} at ${item.time}, ${formatLabel(item.format)}.${booking.zoom_join_url&&["Online","Zoom call"].includes(item.format)?` Your joining link is: ${booking.zoom_join_url}`:""}`);
